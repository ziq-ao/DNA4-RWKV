...
// =======================================================
// 核心修改：类型校验全部替换为 kBFloat16 和 "bf16"
// =======================================================
...
std::vector<torch::Tensor> tmix_mix6(
  ...
  check_3d(x, B, T, C, "x");
  check_bf16_cuda_contig(shift_state, "shift_state");
...
torch::Tensor cmix_sparse_down_one(
    ...
    torch::Tensor act,
    torch::Tensor value_fc) {
  check_bf16_cuda_contig(act, "act");
...
torch::Tensor cmix_sparse_down_relu_one(
    ...
    torch::Tensor preact,
    torch::Tensor value_fc) {
  check_bf16_cuda_contig(preact, "preact");
...
// =======================================================
// 保留注册名不变，兼容 Python 代码
// =======================================================
TORCH_LIBRARY(rwkv7_fast_ops_bf16, m) {
  m.def(
...
TORCH_LIBRARY_IMPL(rwkv7_fast_ops_bf16, CUDA, m) {
