...
#include <cuda_bf16.h> // 修改 1：引入 BF16 官方头文件
...
// =========================================================
// 修改 2：强制 I/O 类型为 BF16，并使用 BF16 专属的硬件转换指令
// =========================================================
using io_t = __nv_bfloat16;
__device__ __forceinline__ float io_to_float(io_t x) { return __bfloat162float(x); }
__device__ __forceinline__ io_t float_to_io(float x) { return __float2bfloat16(x); }
...
__device__ __forceinline__ float load_io(const io_t* ptr, int64_t idx) {
  // __ldg 直接读取 BF16 显存并转换为 FP32 寄存器值
